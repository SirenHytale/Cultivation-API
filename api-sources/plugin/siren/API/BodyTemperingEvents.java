...
/**
 * Events for body tempering (炼体) - the ladder climbed by taking blows rather
 * than by gathering Qi.
 *
 * <h2>Why an addon would care</h2>
 *
 * <p>Tempering is the one Cultivation track whose income is combat damage the
 * player <em>received</em>, which makes it the natural hook for anything that
 * wants to reward endurance: a "never wore armor" achievement, a martial school
 * that only trains bare-skinned, a mode where tempering is the only progression
 * on the server.</p>
 *
 * <p>{@link PreXpGainEvent} can scale or refuse the XP a blow is worth, which is
 * how an addon adds its own multiplier without reimplementing the armor
 * scan. {@link PreLevelUpEvent} can hold a body at its current level - the XP
 * stays banked, so a refusal reads as "not yet" rather than as punishment.</p>
 *
 * <h2>Threading</h2>
 *
 ...
 *
 * <p>Keep them cheap. These fire on every damaging blow a player takes, which is
 * the hottest path this API exposes.</p>
 */
public final class BodyTemperingEvents {

    private BodyTemperingEvents(){}
    ...
    /**
     * About to bank XP for a blow. Unusually, this one is not merely cancellable -
     * it carries a mutable amount, so a listener can scale the reward rather than
     * being limited to allowing or forbidding it.
     *
    ...
    public static final class PreXpGainEvent extends CancellableEvent {
        ...
        /** The level the body is on now - the XP is being banked toward the next one. */
        public int level(){
            return level;
        }

        /** The XP about to be banked, already scaled by armor and by the config rate. */
        public float amount(){
            return amount;
        }
    ...
    public static float firePreXpGain(@Nonnull PreXpGainEvent event){
        if(!EventBus.fire(PRE_XP_GAIN, event, "BodyTemperingPreXpGainEvent")){
            return 0f;
        }
